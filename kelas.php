...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_kelas'])) {
    ...
    $nama_kelas = trim($_POST['nama_kelas']);
    $jumlah_jam = (int)$_POST['jumlah_jam_per_hari'];
    if ($id_kelas > 0 && !empty($nama_kelas) && $jumlah_jam > 0) {
        $stmt = mysqli_prepare($koneksi, "UPDATE tbl_kelas SET nama_kelas = ?, jumlah_jam_per_hari = ? WHERE id_kelas = ?");
        mysqli_stmt_bind_param($stmt, 'sii', $nama_kelas, $jumlah_jam, $id_kelas);
        mysqli_stmt_execute($stmt);
            ...
            <div class="card-body">
                <form action="kelas.php" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label>Jumlah Jam per Hari</label>
                                <input type="number" class="form-control" name="jumlah_jam_per_hari" required min="1" placeholder="Contoh: 10" value="<?php echo $edit_data ? $edit_data['jumlah_jam_per_hari'] : ''; ?>">
                            </div>
                        </div>
                        <div class="col-md-2">
                            <label>&nbsp;</label>
                    ...
                    <?php if ($list_kelas && mysqli_num_rows($list_kelas) > 0): ?>
                        <?php while($row = mysqli_fetch_assoc($list_kelas)): ?>
                            <tr class="kelas-row"
                                ...
                                <td><span class="badge badge-info"><?php echo $row['jumlah_jam_per_hari']; ?> jam</span></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
